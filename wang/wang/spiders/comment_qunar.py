...
from wang.const import qunar_park_id
...
class QunarKashiSpider(scrapy.Spider):
    ...
    def __init__(self, id=None, *args, **kwargs):
        ...
        self.poi = qunar_park_id.get(self.park_name)
        self.url = "https://travel.qunar.com/place/api/html/comments/poi/{}?poiList=true&sortField=1&rank=0&pageSize=10&page={}".format(self.poi, self.id)
    ...
    def parse(self, response):
        ...
        users = selector.xpath(
            '//div[@class="e_comment_usr_name"]/a/text()')
        times = selector.xpath(
            '//div[@class="e_comment_add_info"]/ul/li/text()')
        stars = re.findall(r'cur_star star_\d', data)
        e_comment_content = selector.xpath(
            '//div[@class="e_comment_content"]')
        for one in e_comment_content:
            text = ConvertListToStr(one.xpath('.//p/text()'))
